...
def process_file(input_file_path,output_file_path,dictionary,col_num,wb):
  ...
  if input_file_tag==1:
    ...
    while True:
      ...
      uid,group_name,keywords,province,tier,head,tail,subroots=line.split(unicode(',','utf-8'))
      ...
      new_line="%s,%s,%.9f"%(uid,group_name,y)
...
# -------------------------------修改参数-----------------------------------------------
log_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], "log.txt") # 日志文件记录已处理的文件名称
fin_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'inputfile')
fout_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'outputfile')
# ---------------------------------------------------------------------------------------
